...
async function getHorarioPlanificado(SB, nombre, fecha) {
  // nombre debe ser "Apellido, Nombre" — mismo formato guardado en horarios_semanales
  const lunes  = getLunes(fecha);
  const dayKey = getDayKey(fecha);
  if (!dayKey || !nombre) return null;
  ...
  for (const row of data) {
    ...
    if (tipo === 'libre')   return { entrada:null, salida:null, tipo:'libre' };
    const entrada = diaData.e || '';
    const salida  = diaData.s || '';
    // Si no hay horario fijo cargado, continuar buscando
