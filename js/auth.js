/* auth.js — OSPA */
const ADMIN_CREDS = { user: 'admin', pass: 'ospa2027' };
const SUPER_CREDS = { user: 'superadmin', pass: 'SuperAdmin2026!' };
...
const Auth = {
  loginAdmin(u, p) {
    if (ADMIN_CREDS.user === u && ADMIN_CREDS.pass === p) {
      sessionStorage.setItem(SESSION_KEY, JSON.stringify({ empresaId:'ospa', rol:'admin', ts:Date.now() }));
      return true;
    }
    return false;
  },
