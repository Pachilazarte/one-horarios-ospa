...
const Registros = (() => {
  ...
  async function save() {
    ...
    // Calcular tardanza si hay horario planificado
    let tardanza_min = null;
    if (t && t.includes(':') && e) {
      const pe = t.split('→')[0].trim();
      if (pe.match(/^\d{2}:\d{2}$/)) tardanza_min = calcTard(pe, e);
    }

    // Obtener area_id y empleado_id
    const { data: areaData } = await SB.from('areas').select('id').eq('nombre', area).maybeSingle();
    const area_id = areaData?.id || null;

    // Buscar empleado_id por apellido y nombre
    const partesNombre = nombre.split(', ');
    const apellidoBusq = (partesNombre[0]||'').trim();
    const nombreBusq   = (partesNombre[1]||'').trim();
    const { data: empData } = await SB.from('empleados')
      .select('id').eq('apellido', apellidoBusq).eq('nombre', nombreBusq).maybeSingle();
    const empleado_id = empData?.id || null;
    ...
    if (empleado_id) payload.empleado_id = empleado_id;

    const { error } = await SB.from('registros_asistencia').update(payload).eq('id', id);
